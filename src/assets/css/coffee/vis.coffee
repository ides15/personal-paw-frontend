...
Bubbles = () ->
  ...
  updateActive = (id) ->
    ...
    if id.length > 0
      d3.select("#status").html("<h3>You selected the word <span class=\"active\">#{id}</span></h3>")
    else
      d3.select("#status").html("<h3>You did not select any words</h3>")
...
texts = [
  {key:"menu",file:"menu.csv",name:"Dining Commons Menu"}
  {key:"ist",file:"ist.csv",name:"IST Students"}
  {key:"engineering",file:"engineering.csv",name:"Engineering Students"}
  {key:"smeal",file:"smeal.csv",name:"SMEAL Students"}
]
